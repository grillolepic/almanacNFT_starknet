...
const login = async function (context) {
    if (STARKNET?.isConnected) {
        ...
        context.commit('networkName', NETWORK_NAME);
...
const setAlmanac = async function setAlmanac(context, almanacConfig) {
    ...
    if (ALMANAC_INPUT.market != LAST_ALMANAC_INPUT.market || ALMANAC_INPUT.daysSince != LAST_ALMANAC_INPUT.daysSince) {
        ...
        MILESTONES = [[0,277],[0,304],[0,368],[0,377],[0,981],[0,1025],[0,1039],[0,1096],[0,1135],[0,1207],[0,1254],[0,1416],[0,1498],[0,1792],[1,2157],[0,2178],[0,2256],[1,2282],[1,2349],[1,2394],
        ...
        for (let i=0; i<MILESTONES.length; i++) {
            if (ALMANAC_INPUT.market == MILESTONES[i][0] &&
                ALMANAC_INPUT.daysSince == MILESTONES[i][1]) {
                    blockMilestone = true;
        }}
